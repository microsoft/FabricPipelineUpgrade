...
namespace FabricUpgradePowerShellModule
{
    ...
    /// <summary>
    /// Import Synapse pipeline resources directly from Azure Synapse Analytics workspace using REST APIs.
    /// </summary>
    [Cmdlet(VerbsData.Import, "SynapseWorkspace")]
    public class ImportSynapseWorkspace : PSCmdlet
    {
        [Parameter(
            Position = 0,
            ValueFromPipeline = true,
        ...
        [Parameter(Mandatory = true, HelpMessage = "Synapse workspace name.")]
        public string WorkspaceName { get; set; }

        [Parameter(Mandatory = true, HelpMessage = "Synapse access token for artifact operations (scope: https://dev.azuresynapse.net/). Accepts string, SecureString, or object with AccessToken/Token property (e.g. output of Get-AzAccessToken).")]
        public object SynapseToken { get; set; }

        [Parameter(Mandatory = true, HelpMessage = "ARM access token for workspace metadata operations (scope: https://management.azure.com/). Accepts string, SecureString, or object with AccessToken/Token property (e.g. output of Get-AzAccessToken).")]
        public object ArmToken { get; set; }

        [Parameter(Mandatory = false, HelpMessage = "Optional specific pipeline name to import (imports pipeline and dependencies). If omitted, all pipelines are imported.")]
        public string PipelineName { get; set; }

        [Parameter(Mandatory = false, HelpMessage = "Include datasets and linked services that are not used by any pipelines. Useful for full workspace upgrades.")]
        public SwitchParameter IncludeUnusedResources { get; set; }
        ...
        protected override void ProcessRecord()
        {
            string plainSynapseToken = TokenUnwrapper.Unwrap(SynapseToken, nameof(SynapseToken));
            string plainArmToken = TokenUnwrapper.Unwrap(ArmToken, nameof(ArmToken));
            Console.WriteLine($"Importing from Synapse Workspace: {this.WorkspaceName} in resource group: {this.ResourceGroupName}");

            try
            {
                var cancellationToken = PowerShellCancellationHelper.CreateCancellationToken(this);

                var task = new FabricUpgradeHandler(this.EnableVerboseLogging).ImportSynapseWorkspaceAsync(
                    this.Progress,
                    this.SubscriptionId,
                    this.ResourceGroupName,
                    this.WorkspaceName,
    ...
    public class ConvertToPipeline : Cmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            Console.WriteLine("Converting imported pipeline resources to Fabric pipeline definitions...");
    ...
    /// Supports source ADF factory or Synapse workspace resource IDs.
    /// </summary>
    ...
    public class ExportFabricPipeline : PSCmdlet
    {
        ...
        [Parameter(Mandatory = false, HelpMessage = "Azure Resource ID of the source Azure Data Factory OR Synapse Workspace (e.g., '/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DataFactory/factories/{factoryName}' or '/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Synapse/workspaces/{workspaceName}'). Required for workspace creation. The capacity will be created in the same subscription and resource group as the source.")]
                ...
                else
                {
                    ...
                    if (EnableVerboseLogging)
                    {
                        if (hasCapacityName)
                        {
                            ...
                            Console.WriteLine($"Source Resource ID: {FactoryResourceId ?? "(required for workspace creation)"}");
