...
{
    /// <summary>
    /// Upgrades ADF ADX Command activity to a Fabric KQL activity.
    /// This version brings in resolve steps from the referenced linked service and
    /// writes them into the Fabric activity so that the connection/database is selected.
    /// </summary>
    public class AzureDataExplorerCommandActivityUpgrader : ActivityUpgrader
    {
        // JSON paths for key properties in the ADF Stored Procedure activity.
        private const string adfADXCommandPath = "typeProperties.command";
        ...
        public AzureDataExplorerCommandActivityUpgrader(string parentPath, JToken activityToken, IFabricUpgradeMachine machine)
            : base(ActivityTypes.AzureDataExplorerCommand, parentPath, activityToken, machine)
        {
        }
        ...
        public override void Compile(AlertCollector alerts)
        {
            ...
            this.CheckRequiredAdfProperties(new List<string> { adfADXCommandPath, adfLinkedServiceReferencePath }, alerts);
        }
        ...
        protected override Symbol BuildActivitySymbol(Dictionary<string, JToken> parameterAssignments, AlertCollector alerts)
        {
            ...
            copier.Copy(adfADXCommandPath, allowNull: false);
            ...
            copier.Copy("typeProperties.commandTimeout", copyIfNull: false);
