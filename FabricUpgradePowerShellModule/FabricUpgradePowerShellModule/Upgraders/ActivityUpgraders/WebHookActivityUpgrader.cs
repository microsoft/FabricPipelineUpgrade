...
{
    public class WebHookActivityUpgrader : ActivityUpgrader
    {
        private const string adfUrlPath = "typeProperties.url";
        private const string adfMethod = "typeProperties.method";
        private const string adfBody = "typeProperties.body";
        ...
        public WebHookActivityUpgrader(
            string parentPath, 
            JToken adfActivityToken, 
            IFabricUpgradeMachine machine) : base(ActivityUpgrader.ActivityTypes.WebHook, parentPath, adfActivityToken, machine)
        {
        }
        ...
        public override void Compile(AlertCollector alerts)
        {
            ...
            {
                alerts.AddPermanentError($"Cannot upgrade WebHook activity '{this.Path}' because it includes LinkedServices");
            }

            if (this.CountArrayElements("typeProperties.datasets") > 0)
            {
                alerts.AddPermanentError($"Cannot upgrade WebHook activity '{this.Path}' because it includes Datasets");
            }
        }
            ...
            AlertCollector alerts)
        {
            ...
            copier.Copy("typeProperties.body", allowNull: false);
            copier.Copy("typeProperties.timeout");
            copier.Copy("typeProperties.disableCertValidation");
            copier.Copy("typeProperties.reportStatusOnCallBack");
        ...
        {
            // We cannot upgrade a WebHook that has an Expression for its URL.
            // The ADF WebHook directly declares its URL, but the Fabric WebHook uses a Connection instead.
            // If the ADF WebHook's URL is a String, then we can make this work with a special UrlHostToConnection Resolution.
            ...
            if (urlToken.Type != JTokenType.String)
            {
                alerts.AddPermanentError(
                    $"{this.Path}.typeProperties.url is not a string and the FabricUpgrader cannot upgrade a WebHook activity whose URL is not a string.");
                return;
            }
